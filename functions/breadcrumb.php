...
if (!function_exists('dariah_breadcrumbs')) {
function dariah_breadcrumbs() {
  ...
  if (is_home() || is_front_page()) {
    ...
    if ( is_category() ) {
      ...
      echo '<span>' . get_the_time('Y') . '</span>';
        ...
        $slugs = explode('/', $slug['slug']);
        if (count($slugs) > 1) {
          $path = '';
          for ($p = 0; $p < count($slugs); $p++) {
            $path .= $slugs[$p];
            $page = get_page_by_path($path);
            if (!isset($page)) {
              $page = get_page_by_path($path . '-list');
  }
        ...
        if ($showCurrent == 0 && !empty( $cats )) {
          $cleanCats = explode($delimiter, $cats);
          if (strlen(trim($cleanCats[count($cleanCats) -1])) === 0) {
            array_pop($cleanCats);
          }
          $cats = implode($delimiter, $cleanCats);
        }
