...
<div class="page_title_ctn">
  <div class="wrapper table_wrapper">
    <h1 itemprop="name"><?php echo __('Search Results','thefoxwp'); ?></h1>
    <div id="breadcrumbs">
...
<div id="fw_c" class="clearfix index_page">
    <?php } ?>
     ...
     <h1 itemprop="name"><?php echo esc_html($count).' '.__('results for','thefoxwp').' "<strong>'.$key.'</strong>"'; ?></h1>
      ...
      <div class="masonry_ctn blog_masonry search_masonry">
        ...
        <?php while (have_posts()) : the_post(); ?>
				<?php
					...
					$more = false;
					?>
        <div class="ajax_post post_masonry">
          <div class="masonry_post_wrapper">
						<?php
							$image_url = wp_get_attachment_image_src( get_post_thumbnail_id(), 'medium' );
							$imagePath = !empty( $image_url[0]) ? $image_url[0] : get_stylesheet_directory_uri() . "/images/default-search.jpg";
						?>
	            ...
	            <div class="post-title">
	              <h2><a href="<?php the_permalink() ?>" title="<?php the_title(); ?>">
	                <?php the_title(); ?>
				     ...
				     <div class="post-info"><?php echo __('By','thefoxwp') ?> <?php the_author_posts_link(); ?><?php if (get_post_type($post->ID) == 'post') { ?>    |    <?php the_date( 'd M Y' ); ?> <?php } ?></div>
						 <?php if (strlen(trim(get_the_content(''))) != 0) { ?>
						 <p class="description"><?php echo wp_strip_all_tags(wp_trim_words(get_the_content(''), 40, '...' )); ?></p>
						 <?php } ?>
						</div>
          </div>
        </div>
