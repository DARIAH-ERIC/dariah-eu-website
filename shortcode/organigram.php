            ...
            if (is_array($imageClasses)) {
                ...
                if (count($imageClasses) == count($imageClasses, COUNT_RECURSIVE)) {
                    foreach ($imageClasses as $k => $v) {
                        $personObject->imageClasses .= $v . ' ';
                    }
                } else {
                    $personObject->imageClasses = implode($imageClasses, ' ');
                }
            } else {
                $personObject->imageClasses = $imageClasses;
            }

            $personObject->excerpt = $excerpt;
            $personObject->full = $stripedContent;
            $personObject->displayMore = $displayMore;
            if (strlen(trim($custom['position'][0])) != 0) {
                $personObject->position = $custom['position'][0];
            }
            if ($isLink) {
                $personObject->link = $person->link;
            }
            if ($isSkills) {
                $personObject->skills = $person->skills;
            }
